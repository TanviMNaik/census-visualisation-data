...
st.title('Census Visualisation Data Web App')
...
if 'Pie chart' in plot_list:
  ...
  plt.pie(pie_income, labels = pie_income.index, autopct = '%1.2f%%', startangle = 30, explode = np.linspace(.06, .16, 6)
  plt.title("Distribution of records for income-groups")
  st.pyplot()
  
  pie_gender = census_df['gender'].value_counts()
  plt.pie(pie_gender, labels = pie_gender.index, autopct = '%1.2f%%', startangle = 30, explode = np.linspace(.06, .16, 6)
